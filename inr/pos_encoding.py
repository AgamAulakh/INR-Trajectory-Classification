...
class PosEncodingNeRFOptimized(PosEncodingNeRF):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.exp_i_pi = torch.cat(
            [
                2 ** torch.arange(i, dtype=torch.float32, device=device)[None]
                * self.freq_scale
                * np.pi
                for i in self.num_frequencies
            ],
            dim=1,
        )
        self.out_dim = self.in_features + 2 * np.sum(self.num_frequencies)
    ...
    def forward(self, coords):
        coords_ = torch.cat(
            [
                torch.tile(coords[..., j : j + 1], (1, n))
                for j, n in enumerate(self.num_frequencies)
            ],
            dim=-1,
        )
        exp_i_pi = torch.tile(self.exp_i_pi, (coords_.shape[0], 1))
        exp_i_pi = einops.rearrange(exp_i_pi, "m n -> m 1 n")
...
class PosEncodingGaussian(PosEncodingNone):
    ...
    num_frequencies is embedding size in tancik implementation
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.B_gauss = (
            torch.normal(
                0.0,
                1.0,
                size=(self.in_features, self.num_frequencies[0]),
                requires_grad=False,
            ).to(device)
            * self.freq_scale
        )
        self.B_gauss_pi = 2.0 * np.pi * self.B_gauss
        self.out_dim = self.in_features + 2 * self.num_frequencies[0]
